...
export function createReadOnlyGuard({ effectiveActive, blockedTools, allowedTools, plugin }) {
  ...
  return function askModeReadOnlyGuard(execution) {
    ...
    return `${plugin}: ask mode is read-only, so the "${name}" tool is blocked for this turn. `
      + 'Answer from what you can inspect instead, and tell the user to send the change as a normal message (without /ask), or to run /ask off to leave ask mode early.';
  };
}
