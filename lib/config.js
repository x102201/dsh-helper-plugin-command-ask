...
/**
 * The default `ask:policy` guidance.
 *
 * Ask mode is Cursor's "Ask" collaboration stance: a persistent read-only Q&A
 * mode. The text is deliberately imperative and explicit about the two things
 * models get wrong — treating a follow-up request as permission to act, and
 * "working around" a denied tool.
 */
export const DEFAULT_SECTION = `You are in ask mode: answer the user's questions about this workspace, the code in it, and the running session, and change nothing.
...
export function resolveConfig(raw) {
  ...
  return Object.freeze({
    ...
    section: section === undefined ? DEFAULT_SECTION : section,
    ...
    /** Leave plan mode when ask mode is entered. */
    supersedePlanMode: readBoolean(config, 'supersedePlanMode', true),
    /** Inject a user-switch notice when the mode changes between turns. */
    narrate: readBoolean(config, 'narrate', true),
  });
}
